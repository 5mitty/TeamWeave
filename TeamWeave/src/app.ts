...
async function viewRoles(): Promise<void> {
    console.log(chalk.blue('\nViewing all roles...\n'));
    try {
        const result: QueryResult = await pool.query('SELECT r.id, r.title AS Role, r.salary AS Salary, d.name AS Department FROM role r JOIN department d ON r.department_id = d.id');
...
async function addRole(): Promise<void> {
    ...
    try {
        ...
        const answers: { title: string; salary: string; department_id: number } = await inquirer.prompt([
            {
                type: 'input',
                name: 'title',
